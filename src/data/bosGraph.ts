...
const stubNodes: AirportNode[] = [
  { id: 'A_ENTRY', label: 'Terminal A Entry', terminal: 'A', x: 160, y: 310, kind: 'entry' },
  { id: 'A7', label: 'Gate A7', terminal: 'A', x: 350, y: 280, kind: 'gate' },
  { id: 'A12', label: 'Gate A12', terminal: 'A', x: 440, y: 330, kind: 'gate' },
  { id: 'B_ENTRY', label: 'Terminal B Entry', terminal: 'B', x: 160, y: 310, kind: 'entry' },
  { id: 'B21', label: 'Gate B21', terminal: 'B', x: 360, y: 290, kind: 'gate' },
  { id: 'B29', label: 'Gate B29', terminal: 'B', x: 460, y: 350, kind: 'gate' },
  { id: 'E_ENTRY', label: 'Terminal E Entry', terminal: 'E', x: 160, y: 310, kind: 'entry' },
  { id: 'E5', label: 'Gate E5', terminal: 'E', x: 370, y: 290, kind: 'gate' },
  { id: 'E10', label: 'Gate E10', terminal: 'E', x: 460, y: 350, kind: 'gate' },
];
...
const entryNodeByTerminal: Record<Terminal, Record<EntryPoint, string>> = {
  A: { curbside: 'A_ENTRY', garage: 'A_ENTRY', rental: 'A_ENTRY', rideshare: 'A_ENTRY' },
  B: { curbside: 'B_ENTRY', garage: 'B_ENTRY', rental: 'B_ENTRY', rideshare: 'B_ENTRY' },
  C: { curbside: 'C_CURB', garage: 'C_GARAGE', rental: 'C_RENTAL', rideshare: 'C_RIDE' },
  E: { curbside: 'E_ENTRY', garage: 'E_ENTRY', rental: 'E_ENTRY', rideshare: 'E_ENTRY' },
};

export function getEntryNode(terminal: Terminal, entryPoint: EntryPoint): string {
  return entryNodeByTerminal[terminal][entryPoint];
}

export function getTerminalGraph(terminal: Terminal): { nodes: AirportNode[]; edges: AirportEdge[] } {
  return {
    nodes: bosNodes.filter((node) => node.terminal === terminal),
    edges: bosEdges.filter((edge) => {
      const fromNode = bosNodes.find((node) => node.id === edge.from);
      return fromNode?.terminal === terminal;
    }),
  };
}
